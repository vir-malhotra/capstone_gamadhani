...
BEST_CKPT  = "checkpoints/hmr_extracted_beats/best.ckpt"
NUM_STEPS      = 100
...
class BeatConditionedUNet(nn.Module):
    ...
    def sample(self, beat: torch.Tensor, num_steps: int = NUM_STEPS,
        ...
        """
        Generate a pitch contour conditioned on beat.

        Args:
            beat:  [B, 1, T] beat pulse signal
            prime: [B, 1, P] optional seed — first P frames are clamped at each
                   diffusion step (same mechanism as GaMaDHaNi's sample_fn).
                   P must be <= T.
        """
        b = beat.shape[0]
...
def evaluate():
    ...
    # Val dataset — extracted beats for conditioning
    val_ds = HMRBeatDataset(pitch_task_fn, pitch_qt, split="val", val_ratio=0.1, seed=42)
    print(f"Val windows: {len(val_ds)}")

    # GT beat frames for F1 evaluation (from GT LMDB, not extracted)
    import lmdb as lmdb_mod
    from gamadhani.src.protobuf.data_example import AudioExample as AE
    GT_LMDB_VAL = "/home/vm2426/HMR_processed/lmdb/val"
    gt_beat_map = {}   # uid -> full beat array [T]
    env = lmdb_mod.open(GT_LMDB_VAL, lock=False, readahead=False)
    with env.begin() as txn:
        for key in txn.cursor().iternext(values=False):
            uid = key.decode("ascii")
            ae  = AE(txn.get(key))
            gt_beat_map[uid] = ae.as_dict()["beat"]["data"].astype(np.float32)[0]  # pulse ch
    env.close()
    print(f"Loaded GT beats for {len(gt_beat_map)} recordings")
    ...
    for i in range(len(val_ds)):
        item = val_ds[i]
        beat_gt = item["beat"].unsqueeze(0).unsqueeze(0).to(device)  # [1,1,T] — extracted beats

        # GT beat frames for F1 (from GT annotations, aligned to same window)
        uid   = item["uid"]
        start = item["start"]
        SEQ   = item["beat"].shape[0]
        gt_pulse = gt_beat_map.get(uid)
        if gt_pulse is not None:
            win = gt_pulse[start:start+SEQ]
            beat_frames = np.where(np.diff(np.concatenate([[0], (win>0.5).astype(int)]))==1)[0]
        else:
            beat_frames = np.where(item["beat"].numpy() > 0)[0]  # fallback
