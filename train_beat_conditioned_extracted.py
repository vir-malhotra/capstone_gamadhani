...
class BeatConditionedUNet(nn.Module):
    ...
    def loss(self, x, beat):
        ...
        pred   = self.forward(x_t, t, pb, drop=self.training)
        target = x - self.unet.unpad(noise, pad)
        return torch.mean((self.unet.unpad(pred, pad) - target) ** 2)

    def sample(self, beat: torch.Tensor, num_steps: int = 100):
        self.eval()
        ...
        with torch.no_grad():
            for t in np.linspace(0, 1, num_steps + 1)[:-1]:
                ...
                pred = self.forward(padded_noise, t_tensor * t_array, padded_beat, drop=False)
...
def train(args):
    ...
    wandb.init(
        ...
        config={
            ...
            "beat_dropout": args.beat_dropout,
    ...
    beat_model = BeatConditionedUNet(
        pitch_model, beat_dim=args.beat_dim, beat_dropout=args.beat_dropout
    ).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_name",     default="hmr-extracted-beats-1ch")
    parser.add_argument("--epochs",       type=int,   default=100)
    parser.add_argument("--batch_size",   type=int,   default=16)
    parser.add_argument("--lr",           type=float, default=1e-4)
    parser.add_argument("--beat_dim",     type=int,   default=1)
    parser.add_argument("--beat_dropout", type=float, default=0.1)
    parser.add_argument("--val_ratio",    type=float, default=0.1)
    parser.add_argument("--seed",         type=int,   default=42)
    parser.add_argument("--save_every",   type=int,   default=10)
    parser.add_argument("--num_workers",  type=int,   default=4)
    parser.add_argument("--ckpt_dir",     default="checkpoints/hmr_extracted_beats")
